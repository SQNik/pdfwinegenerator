...
export class PDFService {
  ...
  async generatePDFFromHTMLWithCollection(
    ...
    try {
      ...
      if (options.customFormat) {
        ...
        // Apply custom margins if they exist
        if (format.margins) {
          pdfOptions.margin = {
            top: `${format.margins.top}${format.unit}`,
            right: `${format.margins.right}${format.unit}`,
  ...
  private injectCollectionDataToHTML(htmlContent: string, collectionData: any): string {
    ...
    try {
      ...
      // Handle direct collection.xxx replacements (shorthand for dynamic fields by name)
      // This provides the simplest syntax: {{collection.okladka}} instead of {{collection.dynamicFieldsByName.okladka}}
      // Only match single-level properties to avoid conflicts with collection.name, collection.id, etc.
      const reservedFields = ['id', 'name', 'description', 'wines', 'wineslist', 'totalwines', 'customtitle', 'dynamicfields', 'dynamicfieldsbyname', 'createdat', 'updatedat', 'tags', 'status', 'winecount', 'winesbycategory'];
      processedHTML = processedHTML.replace(/\{\{collection\.([\w_]+)\}\}/g, (match, fieldName) => {
        // Skip reserved collection properties
        if (reservedFields.includes(fieldName.toLowerCase())) {
          return match; // Keep original placeholder for reserved fields
        }
        
        // Try to find in dynamicFieldsByName
        const dynamicFieldsByName = collectionData.dynamicFieldsByName || {};
        ...
        // If not found, return empty string (field doesn't exist)
        return '';
      });
