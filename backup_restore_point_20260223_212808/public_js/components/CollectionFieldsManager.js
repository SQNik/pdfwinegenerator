...
class CollectionFieldsManager {
    ...
    async renderFieldsList() {
        ...
        try {
            ...
            container.innerHTML = sortedFields.map(field => this.renderFieldRow(field)).join('');
        } catch (error) {
            console.error('CollectionFieldsManager: Błąd renderowania listy pól:', error);
            container.innerHTML = '<tr><td colspan="7" class="alert alert-danger">Błąd ładowania pól kolekcji</td></tr>';
        }
    }
    ...
    renderFieldRow(field) {
        ...
        const stepLabel = field.step ? `Krok ${field.step}` : 'Krok 1';
        const requiredIcon = field.required ? '<i class="bi bi-check-circle-fill text-success"></i>' : '<i class="bi bi-circle text-muted"></i>';
        ...
        return `
            <tr data-field-id="${field.id}">
                ...
                <td>
                    <i class="${typeInfo.icon} text-primary"></i> ${typeInfo.label}
                </td>
                <td><span class="badge bg-primary">${stepLabel}</span></td>
                <td class="text-center">${requiredIcon}</td>
                <td>
    ...
    async loadFieldToModal(fieldId) {
        try {
            ...
            document.getElementById('collectionFieldStep').value = field.step || 1; // Domyślnie krok 1
    ...
    resetFieldModal() {
        ...
        // Nie wymuszamy wartości - form.reset() ustawi selected z HTML (krok 1)
    ...
    async saveField() {
        ...
        const stepValue = formData.get('fieldStep');
        const stepNumber = parseInt(stepValue);
        
        console.log('CollectionFieldsManager: Odczytywanie kroku z formularza:', {
            raw: stepValue,
            parsed: stepNumber,
            isNaN: isNaN(stepNumber)
        });
        ...
        const fieldData = {
            ...
            step: !isNaN(stepNumber) && stepNumber > 0 ? stepNumber : 1 // Domyślnie krok 1 (zgodnie z HTML)
        };
