...
const collectionWizardSteps = [
    ...
    // Step 3: Product Selection
    {
        ...
        renderFunction: async (container, data, wizard) => {
            ...
            // Load grid preferences from localStorage
            const savedColumns = localStorage.getItem('wizard-grid-columns') || '3';
            const savedItemsPerPage = localStorage.getItem('wizard-items-per-page') || '12';
            ...
            container.innerHTML = `
                ...
                <!-- Grid Controls -->
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1rem; padding: 0.75rem; background: white; border-radius: 0.5rem; border: 1px solid #e5e7eb;">
                    <div style="display: flex; align-items: center; gap: 0.75rem;">
                        <span style="font-size: 0.875rem; font-weight: 600; color: #64748b;">
                            <i class="bi bi-grid-3x3-gap"></i> Kolumny:
                        </span>
                        <div class="btn-group" role="group">
                            <button type="button" class="ds-btn ds-btn-sm ${savedColumns === '2' ? 'ds-btn-primary' : 'ds-btn-ghost'}" data-columns="2" id="grid-col-2">
                                <i class="bi bi-grid"></i> 2
                            </button>
                            <button type="button" class="ds-btn ds-btn-sm ${savedColumns === '3' ? 'ds-btn-primary' : 'ds-btn-ghost'}" data-columns="3" id="grid-col-3">
                                <i class="bi bi-grid-3x2"></i> 3
                            </button>
                            <button type="button" class="ds-btn ds-btn-sm ${savedColumns === '4' ? 'ds-btn-primary' : 'ds-btn-ghost'}" data-columns="4" id="grid-col-4">
                                <i class="bi bi-grid-3x3"></i> 4
                            </button>
                        </div>
                    </div>
                    <div style="display: flex; align-items: center; gap: 0.75rem;">
                        <span style="font-size: 0.875rem; font-weight: 600; color: #64748b;">
                            <i class="bi bi-stack"></i> Na stronę:
                        </span>
                        <select class="ds-select" id="items-per-page" style="width: auto; min-width: 80px; padding: 0.375rem 0.75rem; font-size: 0.875rem;">
                            <option value="6" ${savedItemsPerPage === '6' ? 'selected' : ''}>6</option>
                            <option value="12" ${savedItemsPerPage === '12' ? 'selected' : ''}>12</option>
                            <option value="24" ${savedItemsPerPage === '24' ? 'selected' : ''}>24</option>
                            <option value="48" ${savedItemsPerPage === '48' ? 'selected' : ''}>48</option>
                        </select>
                    </div>
                </div>
                
                <div class="wizard-wine-grid wizard-wine-grid-${savedColumns}" id="wines-grid">
                    <div class="text-center" style="grid-column: 1 / -1;">
                ...
                <!-- Pagination -->
                <div id="pagination-container" style="display: flex; justify-content: center; align-items: center; gap: 1rem; margin-top: 1.5rem; padding: 1rem; background: white; border-radius: 0.5rem; border: 1px solid #e5e7eb;">
                    <button type="button" class="ds-btn ds-btn-ghost ds-btn-sm" id="prev-page-btn" disabled>
                        <i class="bi bi-chevron-left"></i> Poprzednia
                    </button>
                    <div id="page-info" style="font-size: 0.875rem; font-weight: 600; color: #64748b;">
                        Strona 1 z 1
                    </div>
                    <button type="button" class="ds-btn ds-btn-ghost ds-btn-sm" id="next-page-btn" disabled>
            ...
            let filteredWines = [];
            let currentPage = 1;
            let itemsPerPage = parseInt(savedItemsPerPage);
            let gridColumns = parseInt(savedColumns);
            ...
            function renderWines() {
                ...
                const endIndex = Math.min(startIndex + itemsPerPage, filteredWines.length);
                const paginatedWines = filteredWines.slice(startIndex, endIndex);
                
                winesGrid.innerHTML = paginatedWines.map(wine => {
                ...
                winesGrid.querySelectorAll('.wizard-wine-card').forEach(card => {
                    card.addEventListener('click', () => {
                        ...
                        const wine = allWines.find(w => w.catalogNumber === catalogNumber);
            ...
            function updatePagination() {
                ...
                if (pageInfo) {
                    ...
                    const endIndex = Math.min(currentPage * itemsPerPage, filteredWines.length);
                    pageInfo.textContent = `Strona ${currentPage} z ${totalPages} (${startIndex}-${endIndex} z ${filteredWines.length})`;
                }
            ...
            container.querySelectorAll('[data-columns]').forEach(btn => {
                btn.addEventListener('click', () => {
                    const columns = btn.dataset.columns;
                    gridColumns = parseInt(columns);
                    localStorage.setItem('wizard-grid-columns', columns);
                    
                    // Update button states
                    container.querySelectorAll('[data-columns]').forEach(b => {
                        b.classList.remove('ds-btn-primary');
                        b.classList.add('ds-btn-ghost');
                    });
                    btn.classList.remove('ds-btn-ghost');
                    btn.classList.add('ds-btn-primary');
                    
                    // Update grid class
                    const grid = container.querySelector('#wines-grid');
                    grid.className = `wizard-wine-grid wizard-wine-grid-${columns}`;
                });
            });
            ...
            container.querySelector('#items-per-page').addEventListener('change', (e) => {
                ...
                localStorage.setItem('wizard-items-per-page', e.target.value);
                currentPage = 1; // Reset to first page
                renderWines();
                updatePagination();
            });
            ...
            container.querySelector('#prev-page-btn').addEventListener('click', () => {
                if (currentPage > 1) {
                    ...
                    container.querySelector('#wines-grid').scrollIntoView({ behavior: 'smooth', block: 'start' });
                }
            });
                                    ...
                                    return `
                                        <li class="wizard-summary-item" 
                                            ...
                                            <div class="wizard-summary-inputs">
                                                <div>
                                                    <label style="font-size: 0.75rem; color: #718096;">Cena 1 (zł)</label>
                                                ...
                                                <div>
                                                    <label style="font-size: 0.75rem; color: #718096;">Pojemność 1 (ml)</label>
                                                    <input type="number" class="ds-input wizard-summary-input" 
                                                ...
                                                <div>
                                                    <label style="font-size: 0.75rem; color: #718096;">Cena 2 (zł)</label>
                                                    <input type="number" class="ds-input wizard-summary-input" 
                                                           data-catalog-number="${wine.catalogNumber}" data-field="price2"
                                                           value="${details.price2 || parsePrice(wine.price2)}" placeholder="0.00" step="0.01" min="0">
                                                </div>
                                                <div>
                                                    <label style="font-size: 0.75rem; color: #718096;">Pojemność 2 (ml)</label>
                                                    <input type="number" class="ds-input wizard-summary-input" 
                                                           data-catalog-number="${wine.catalogNumber}" data-field="volume2"
                                                           value="${details.volume2 || parseVolume(wine.poj2)}" placeholder="0" step="1" min="0">
                                                </div>
                                            </div>
                                        </li>
                                    `;
